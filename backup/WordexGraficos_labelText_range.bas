...
'   - label sai sempre de Range.Text. Para totais calculados em memória,
'     usa-se uma célula temporária com a formatação da célula de origem.
' =====================================================================
...
Private Function WX_MontarJsonGrafico( _
    ...
    For i = 1 To ordem.Count
        ...
        label = WX_ObterLabelAgregado(stats, operacao, plan, colunaValor)
...
Private Function WX_NovoStats() As Object
    ...
    stats.Add "Min", Empty
    stats.Add "Max", Empty
    stats.Add "FirstText", vbNullString
    stats.Add "MinText", vbNullString
    stats.Add "MaxText", vbNullString
    stats.Add "FormatAddress", vbNullString
...
Private Sub WX_Acumular(ByRef cel As Range, ByRef stats As Object)
    ...
    If IsNumeric(valor) Then
        ...
        If CLng(stats("Count")) = 0 Then
            stats("FirstText") = CStr(cel.Text)
            stats("FormatAddress") = cel.Address(False, False)
        End If
        ...
        If IsEmpty(stats("Min")) Then
            stats("Min") = numero
            stats("Max") = numero
            stats("MinText") = CStr(cel.Text)
            stats("MaxText") = CStr(cel.Text)
        Else
            If numero < CDbl(stats("Min")) Then
                stats("Min") = numero
                stats("MinText") = CStr(cel.Text)
            End If

            If numero > CDbl(stats("Max")) Then
                stats("Max") = numero
                stats("MaxText") = CStr(cel.Text)
            End If
        End If
...
Private Function WX_ObterLabelAgregado( _
    ...
    Select Case operacao
        Case "Min"
            WX_ObterLabelAgregado = CStr(stats("MinText"))

        Case "Max"
            WX_ObterLabelAgregado = CStr(stats("MaxText"))

        Case "Sum", "Avg"
            If CLng(stats("Count")) = 1 Then
                WX_ObterLabelAgregado = CStr(stats("FirstText"))
            Else
                WX_ObterLabelAgregado = WX_LabelPorRangeTemporario(plan, stats, valor, False)
            End If

        Case "Count"
            WX_ObterLabelAgregado = WX_LabelPorRangeTemporario(plan, stats, valor, True)
    End Select
End Function

Private Function WX_LabelPorRangeTemporario( _
    ByRef plan As Worksheet, _
    ByRef stats As Object, _
    ByVal valor As Variant, _
    ByVal inteiro As Boolean _
) As String
    Dim celOrigem As Range
    Dim celTemp As Range

    If IsEmpty(valor) Then
        WX_LabelPorRangeTemporario = vbNullString
        Exit Function
    End If

    Set celTemp = WX_ObterCelulaTemporaria(plan.Parent)
    celTemp.Clear

    If Not inteiro Then
        If stats.Exists("FormatAddress") Then
            If CStr(stats("FormatAddress")) <> vbNullString Then
                Set celOrigem = plan.Range(CStr(stats("FormatAddress")))
                celOrigem.Copy
        ...
        End If
    Else
        celTemp.NumberFormat = "#,##0"
    End If

    celTemp.Value2 = valor
    celTemp.EntireColumn.ColumnWidth = 255

    WX_LabelPorRangeTemporario = CStr(celTemp.Text)
End Function

Private Function WX_ObterCelulaTemporaria(ByRef wb As Workbook) As Range
    Const WX_TEMP_SHEET As String = "_WORDEX_TEMP"

    Dim planTemp As Worksheet

    On Error Resume Next
    Set planTemp = wb.Worksheets(WX_TEMP_SHEET)
    On Error GoTo 0

    If planTemp Is Nothing Then
        Set planTemp = wb.Worksheets.Add(After:=wb.Worksheets(wb.Worksheets.Count))
        planTemp.Name = WX_TEMP_SHEET
        planTemp.Visible = xlSheetVeryHidden
    End If

    Set WX_ObterCelulaTemporaria = planTemp.Range("A1")
End Function
