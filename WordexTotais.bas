Attribute VB_Name = "WordexTotais"
Option Explicit
    ...
    Select Case LCase$(Trim$(tipoTotal))
        ...
        Case Else
            Err.Raise vbObjectError + 3004, "GerarAbaTotais", _
                "Valor a calcular invÃ¡lido: '" & tipoTotal & "'. Use Sum, Min, Max, Avg ou Count."
    End Select
End Function
...
Private Function Totais_ObterOuCriarAba(ByVal nomeAba As String) As Worksheet
    ...
    If Not Totais_ObterOuCriarAba Is Nothing Then
        If MsgBox("A aba '" & nomeAba & "' jÃ¡ existe. Deseja substituÃ­-la?", vbYesNo + vbQuestion, "Wordex") = vbNo Then
            Err.Raise vbObjectError + 3001, "Totais_ObterOuCriarAba", "OperaÃ§Ã£o cancelada."
        End If
...
Private Function Totais_ObterNumeroColuna(ByRef plan As Worksheet, ByVal tituloColuna As String) As Long
    ...
    Err.Raise vbObjectError + 3002, "Totais_ObterNumeroColuna", _
        "Coluna '" & tituloColuna & "' nÃ£o encontrada na aba '" & plan.Name & "'."
End Function
...
Private Function Totais_FormatoAvgSecao(ByVal secao As String) As String
    ...
    If posDecimal > 0 Then
        ' JÃ¡ possui casas decimais. MantÃ©m como estÃ¡.
        Totais_FormatoAvgSecao = secao
    Else
        ' NÃ£o possui casas decimais. Acrescenta uma.
